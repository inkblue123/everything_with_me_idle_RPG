...
在第二章开放村内村外设施之后，在铁匠铺进行合成
允许一样的多个白合成到绿，不改装备性质，只增幅一些属性参数
...
高稀有度的装备当然应该有更强的属性，但是具体怎么个强法，这里有两个方案

方案 1：无脑乘倍率
在基础属性上给每个属性乘以一定的倍率，就是高稀有度装备的属性了
这个倍率一般搞个
100% 110% 130% 160% 200%
普通，优质，稀有，史诗，传说
...
攻击+10，暴击率+10%，攻速-1 秒
优质武器
攻击+12，暴击率+20%，暴击伤害+10%，攻速-1.2 秒
稀有武器
攻击+15，暴击率+30%，暴击伤害+15%，攻速-1.2 秒
暴击时对敌人造成 3 秒的减速效果
史诗武器又在稀有的基础上再变化
大概就这种意思

其实方案 2 就是方案 1 的灵活版本，都能确定每个稀有度的属性了，安排好数值也就是方案 1 的效果
当然，方案 1 的好处就是方便，有些不是那么重要的武器直接自动生成就好，
另外，由于高稀有度武器有全新的属性，而玩家的装备又是可以向上合成的，玩家理应知道自己武器之后阶段会是什么样子
这个功能安排在武器合成的时候吧，平时就不用展示了，怪占空间的

装备除了属性，还有出售购买价值
这个就用方案 1 即可，倍率是
破损，普通，优良，稀有，史诗，传说
50%，100%，120%，150%，200%，500%
...
此外还有一些比例加成的属性，这些属性在加算完毕之后结算，相互之间累加
比如玩家身上有精力充沛 buff，攻击力+20%，同时敌人上了个虚弱 buff，攻击力-20%
最终加成 ratio 就是 0%
结算是 20*（100+ratio）*0.01
...
所以最终伤害就是 4.4\*（100+20）% =5.28
多个伤害增幅相互累加
比如玩家还有对野兽敌人伤害增加 10%，那么最终伤害就是
4.4\*（100+20+10）%=5.78

如果伤害增幅累加的结果是负数，有三个方案
方案 1 符合玩家直觉，就也是照样计算，可以负到-100%以下，然后最终算出来是 0
方案 2 是参考攻速的负数计算，把-100%变成伤害 1/（100- -100） = 1/2，
方案 3 是多个减伤害的数值相互乘算，比如 5 个-20%，就是 0.8*0.8*0.8*0.8*0.8 = 0.327

方案 1 符合玩家直觉，缺点是负数部分总共就只有 100%，减减就没了，担心在内容越来越多的时候没注意就出现伤害变 0 的情况了
方案 2 没什么特别的优缺，甚至对玩家来说，这个算法让减益效果变弱了不少
方案 3 主要是难以统计，
就用方案 2 了，这个算法还会用到生活技能关于伤害计算的地方
...
这个属性开始开发的时候没有设定好，现在和游戏的一些系统有矛盾，在这里规范一下

最开始的攻击间隔照抄梅尔沃放置，玩家只有一个攻击速度属性，装备、技能等东西身上也有攻击速度
计算攻速时就将所有速度全都累加到一起，得到最终的攻击速度，再拿到其他地方使用
这样设计的问题在于，这样的攻速是越小越好，而其他属性是越大越好
在各种处理属性的场合里，攻速总要单独拿出来处理
并且攻速整体来说是一个小数字，加加减减影响很大，会导致后期的数值不好做

新方案打算选择结合明日方舟和梅尔沃放置的攻速
首先每种武器类型都有自己的攻速，称为基础攻击间隔，这和梅尔沃与明日方舟都类似，
之后装备、技能上的攻击速度改成攻击速度加成，变成一个正数，越大越好的那种，这部分来自于明日方舟
算法是基础攻击间隔*（1+攻速加成）*100% = 最终攻击间隔
如果还有固定值的攻速，那就是先加减攻击间隔，再计算百分比，这部分与梅尔沃一样

这样做的好处是攻速也变成了越大越好的属性了，不需要单独处理，而且也会越大收益越低
攻击间隔最低值设定为 0.25s，攻速可以无限叠加
这样到后期的时候，快攻击间隔的武器类型攻速严重溢出，
比如 2s 基础间隔，只需要 300 攻速就能满，此时更多的攻速就没用了，需要另外一些维度的属性来提高输出
慢攻击间隔的武器叠攻速还能吃，但是也肯定不如叠一部分攻击
总体来说玩法有一些区分
后期做一些流派，把锤子攻速加成到很快，或者超级减速匕首，总之有些不一样的花样了

新方案有个新问题是多子类武器的基础攻击间隔
基础攻击间隔按理来说只和单一的一个装备子类绑定，如果一个装备有多个子类，到底选哪个呢

有个做法是把基础攻击间隔放在每个装备身上，改变玩家攻速时直接用这个数值覆盖
另外的方案就是改变玩家攻速时读取武器类型，再通过武器类型读取攻击间隔，再用这个攻击间隔

出问题的主要是后面这个方案，而这个方案一看就很死板
感觉前一个方案就挺好

还有一个问题，玩家主手副手都可以带装备，此时如何选择呢
想做的方案是把装备上的攻击间隔从直接修改变成相对变化
比如玩家基础攻击间隔 3 秒，剑的攻击间隔变化为+2 秒，所以玩家带了剑的时候，基础攻击间隔就是 5 秒
另外剑上有 20%点攻速加成，就照常计算
然后有手弩，攻击间隔变化也是+2，玩家又用剑又用手弩时，最终攻击间隔就是 3+2+2
手弩上的其他属性都正常累加，攻速也加，攻击力也加
玩家当然可以手持两把武器，享受到两把武器的攻击加成，代价是两把武器的基础攻击间隔也会一起吃，值不值得就交给玩家自己取舍了
同理，防具上也可以附带攻击间隔变化，不过这块没有想好，以后做套装的时候再说吧
