...
然后用可探索地点还有距离上次成功时间和探索时间限制的计算
时间成功率的算法和探索力成功率的算法一致

两个成功率相乘，得到最终成功率，进行数值随机，判断是否探索成功
成功时，从奖励列表里选择一个
...
探索采集类技能：
采集探索没有前提，伐木挖矿也没有前提，这俩虽然可以用斧头和稿子来约束，但是感觉不做这个限制会比较友好
潜水还没设定，感觉也是不用前提
