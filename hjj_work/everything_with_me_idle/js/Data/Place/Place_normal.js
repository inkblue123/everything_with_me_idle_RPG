...
function init_village_backhill_normal_place(places) {
    ...
    places[id].add_condition_connect_normal_place('logged_forest', 'unlock_collect_logging', true, '='); //完成了指定事件才允许进入已开荒的林区
    places[id].add_condition_connect_normal_place('forest_edge', 'UGS_pass_road_flag', true, '=', 'forest_edge_road'); //完成了指定事件才允许进入外层森林
    ...
    places[id].set_collect_data(100, 1); //这个地点的采集相关参数，采集防御力100点
    places[id].set_collect_item('red_berry', 100, false); //这个地点可采集的物品，红浆果
    places[id].set_collect_item('yellow_berry', 100, false); //这个地点可采集的物品，黄浆果
    places[id].set_collect_item('black_berry', 100, false); //这个地点可采集的物品，黑浆果
    places[id].set_collect_item('porcini', 1, false); //这个地点可采集的物品，牛肝菌
    ...
    places[id].set_logging_tree('bushes', 10, false); //这个地点可刷的树
    places[id].set_collect_data(10, 0.5); //这个地点的采集相关参数，采集防御力10点
    places[id].set_collect_item('red_berry', 100, false); //这个地点可采集的物品，红浆果
    places[id].set_collect_item('yellow_berry', 100, false); //这个地点可采集的物品，黄浆果
    places[id].set_collect_item('black_berry', 100, false); //这个地点可采集的物品，黑浆果
    places[id].set_collect_item('decayed_wood', 50, false); //这个地点可采集的物品，朽木
    places[id].set_collect_item('broken_fur', 50, false); //这个地点可采集的物品，碎毛皮
    places[id].set_collect_item('hard_rock', 10, false); //这个地点可采集的物品，坚硬岩石
    places[id].set_collect_item('porcini', 5, false); //这个地点可采集的物品，牛肝菌
    places[id].set_collect_item('craterellus', 5, false); //这个地点可采集的物品，喇叭菌
    places[id].set_collect_item('copper_coin', 1, false); //这个地点可采集的物品，铜币
    places[id].set_collect_item('wood_sticks', 1, false, null, null, 'ordinary'); //这个地点可采集的物品，木制棍棒
    places[id].set_collect_item('termite_mushroom', 1, true, 2, 2880); //这个地点可采集的物品，鸡枞
    places[id].set_collect_item('viresilver_stem', 1, true, 1, 1440); //这个地点可采集的物品，绿银草茎
    ...
    places[id].set_collect_data(15, 1); //这个地点的采集相关参数，采集防御力15点
    places[id].set_collect_item('decayed_wood', 50, false); //这个地点可采集的物品，朽木
    places[id].set_collect_item('broken_fur', 50, false); //这个地点可采集的物品，碎毛皮
    places[id].set_collect_item('animal_bone', 10, false); //这个地点可采集的物品，兽骨
    places[id].set_collect_item('Oak_logs', 10, false); //这个地点可采集的物品，橡树原木
    places[id].set_collect_item('porcini', 1, false); //这个地点可采集的物品，牛肝菌
    places[id].set_collect_item('craterellus', 1, false); //这个地点可采集的物品，喇叭菌
    places[id].set_collect_item('wood_sticks', 1, false, null, null, 'ordinary'); //这个地点可采集的物品，木制棍棒
    ...
    places[id].set_collect_data(100, 2); //这个地点的采集相关参数，采集防御力100点
    places[id].set_collect_item('decayed_wood', 100, false); //这个地点可采集的物品，朽木
    places[id].set_collect_item('river_mussel', 20, false); //这个地点可采集的物品，河蚌
    places[id].set_collect_item('river_crab', 20, false); //这个地点可采集的物品，河蟹
    places[id].set_collect_item('coral_fungus', 5, false); //这个地点可采集的物品，珊瑚菌
    places[id].set_collect_item('chanterelle', 5, false); //这个地点可采集的物品，鸡油菌
    ...
    places[id].set_logging_tree('oak_tree', 10, false); //这个地点可刷的树，橡树
    places[id].set_collect_data(120, 1.5); //这个地点的采集相关参数，采集防御力120点
    places[id].set_collect_item('decayed_wood', 50, false); //这个地点可采集的物品，朽木
    places[id].set_collect_item('red_berry', 10, false); //这个地点可采集的物品，红浆果
    places[id].set_collect_item('yellow_berry', 10, false); //这个地点可采集的物品，黄浆果
    places[id].set_collect_item('black_berry', 10, false); //这个地点可采集的物品，黑浆果
    places[id].set_collect_item('Oak_logs', 5, false); //这个地点可采集的物品，橡树原木
    places[id].set_collect_item('porcini', 1, false); //这个地点可采集的物品，牛肝菌
    places[id].set_collect_item('coral_fungus', 1, false); //这个地点可采集的物品，珊瑚菌
    ...
    places[id].set_collect_data(60, 1); //这个地点的采集相关参数，采集防御力60点
    places[id].set_collect_item('decayed_wood', 50, false); //这个地点可采集的物品，朽木
    places[id].set_collect_item('red_berry', 10, false); //这个地点可采集的物品，红浆果
    places[id].set_collect_item('yellow_berry', 10, false); //这个地点可采集的物品，黄浆果
    places[id].set_collect_item('black_berry', 10, false); //这个地点可采集的物品，黑浆果
    ...
    places[id].set_collect_item('craterellus', 1, false); //这个地点可采集的物品，喇叭菌
    places[id].set_collect_item('coral_fungus', 1, false); //这个地点可采集的物品，珊瑚菌
    places[id].set_collect_item('chanterelle', 1, false); //这个地点可采集的物品，鸡油菌
    ...
    places[id].set_logging_tree('bushes', 10, false); //这个地点可刷的树，灌木丛
    places[id].set_logging_tree('Willow_tree', 10, false); //这个地点可刷的树，柳树
    places[id].set_collect_data(200, 2.5); //这个地点的采集相关参数，采集防御力200点
    places[id].set_collect_item('river_crab', 35, false); //这个地点可采集的物品，河蟹
    places[id].set_collect_item('coral_fungus', 15, false); //这个地点可采集的物品，珊瑚菌
    places[id].set_collect_item('chanterelle', 15, false); //这个地点可采集的物品，鸡油菌
    places[id].set_collect_item('viresilver_stem', 1, true, 1, 1440); //这个地点可采集的物品，绿银草茎
    ...
    id = 'forest_core'; //内层森林
    add_normal_Place(places, id, area);
    places[id].add_connect_normal_place('VBH_rest_location', 'alp_cave_inlet', 'FC_scree', 'FC_Peakarea', 'forest_edge');
    places[id].set_live_plan_flag(9); //当前地点可以执行的生活技能
    places[id].set_logging_data(5); //这个地点的伐木相关参数，树的复活时间5秒
    ...
    places[id].set_collect_data(90, 4); //这个地点的采集相关参数，采集防御力90点
    places[id].set_collect_item('decayed_wood', 50, false); //这个地点可采集的物品，朽木
    places[id].set_collect_item('animal_bone', 50, false); //这个地点可采集的物品，兽骨
    places[id].set_collect_item('hard_rock', 50, false); //这个地点可采集的物品，坚硬岩石
    places[id].set_collect_item('pine_logs', 50, false); //这个地点可采集的物品，松树原木
    places[id].set_collect_item('sliver_coin', 20, false); //这个地点可采集的物品，银币
    places[id].set_collect_item('greedy_copper_coin', 5, false); //这个地点可采集的物品，贪婪的铜币
    places[id].set_collect_item('termite_mushroom', 5, true, 3, 1440); //这个地点可采集的物品，鸡枞
    places[id].set_collect_item('frost_marrow_ice', 1, true, 1, 2880); //这个地点可采集的物品，寒髓松冰晶
    ...
    places[id].set_collect_data(300, 4); //这个地点的采集相关参数，采集防御力300点
    places[id].set_collect_item('hard_rock', 5, false); //这个地点可采集的物品，坚硬岩石
    places[id].set_collect_item('lowFe_rock', 1, false); //这个地点可采集的物品，含铁岩石
    ...
    places[id].set_collect_data(300, 5); //这个地点的采集相关参数，采集防御力300点
    places[id].set_collect_item('decayed_wood', 50, false); //这个地点可采集的物品，含铁岩石
    places[id].set_collect_item('pine_logs', 20, false); //这个地点可采集的物品，松树原木
    places[id].set_collect_item('lightning_bark', 5, true, 1, 1440); //这个地点可采集的物品，雷杉树皮
    places[id].set_collect_item('lightning_iron_logs', 1, true, 1, 1440); //这个地点可采集的物品，雷击铁杉木
    places[id].set_collect_item('ice_matsutake', 1, true, 1, 1440); //这个地点可采集的物品，冰松茸
    places[id].set_collect_item('frost_marrow_ice', 1, true, 1, 1440); //这个地点可采集的物品，寒髓松冰晶
}
//后山洞穴区域的普通地点
